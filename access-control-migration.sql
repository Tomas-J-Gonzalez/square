-- Access Control Database Migration for Show Up or Else
-- Run this SQL in your Supabase SQL Editor
-- This migration adds public/private event access controls
...
-- Step 2: Create event_invitees table (for private events)
...
-- Step 3: Create rsvp_tokens table (for private events)
...
-- Step 6: Create RPC functions for access control
...
DROP POLICY IF EXISTS "Event hosts can manage invitees" ON public.event_invitees;
CREATE POLICY "Event hosts can manage invitees" ON public.event_invitees
  FOR ALL USING (
    EXISTS (
      SELECT 1 FROM public.events 
...
DROP POLICY IF EXISTS "Invitees can view their own invitations" ON public.event_invitees;
CREATE POLICY "Invitees can view their own invitations" ON public.event_invitees
  FOR SELECT USING (
...
DROP POLICY IF EXISTS "Event hosts can manage RSVP tokens" ON public.rsvp_tokens;
CREATE POLICY "Event hosts can manage RSVP tokens" ON public.rsvp_tokens
  FOR ALL USING (
    EXISTS (
      SELECT 1 FROM public.events 
...
DROP POLICY IF EXISTS "Anyone can validate RSVP tokens" ON public.rsvp_tokens;
CREATE POLICY "Anyone can validate RSVP tokens" ON public.rsvp_tokens
  FOR SELECT USING (true);

-- Step 8: Update existing events to have default access settings
UPDATE public.events 
SET 
  access = 'private',
  page_visibility = 'private'
WHERE access IS NULL OR page_visibility IS NULL;

-- Migration complete!
-- Access control features are now ready to use.
